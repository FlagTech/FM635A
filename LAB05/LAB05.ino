...
void setup() {
  ...
  uint16_t randSeed = analogRead(A0); // 取得亂數種子
  srand(randSeed);
...
void loop() {
  ...
  ailayer_dense_f32_t   dense_layer_1 = AILAYER_DENSE_F32_A(10); // 第1層隱藏層, 使用10個神經元
  ailayer_tanh_f32_t    tanh_layer_1  = AILAYER_TANH_F32_A();    // 第1層隱藏層的激勵函數, 因為是sine wave的回歸問題, 使用tanh處理
  ailayer_dense_f32_t   dense_layer_2 = AILAYER_DENSE_F32_A(10); // 第2層隱藏層, 使用10個神經元
  ailayer_tanh_f32_t    tanh_layer_2  = AILAYER_TANH_F32_A();    // 第2層隱藏層的激勵函數, 因為是sine wave的回歸問題, 使用tanh處理
  ailayer_dense_f32_t   dense_layer_3 = AILAYER_DENSE_F32_A(1);  // 輸出層, 使用1個神經元
  ailayer_tanh_f32_t    tanh_layer_3  = AILAYER_TANH_F32_A();    // 輸出層的激勵函數, 因為是sine wave的回歸問題, 使用tanh處理

  ailoss_mse_t mse_loss;  // 因為是回歸問題, 損失函數(Loss Function)選擇MSE(均方誤差)
  
  // --------------------------- 定義模型結構 ----------------------------
  aimodel_t model;  // 宣告AIfES模型
  ailayer_t *x;     // 為了連結各層神經層過渡用的變數
  
  // 使用上述定義的神經層組件來加入到模型, 其中會使用到過渡用的變數x
